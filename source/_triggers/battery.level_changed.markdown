...
This fires whenever either device charges back into the 80% to 100% range. The `between` threshold is exclusive, so `value_max` with `number: 101` is used to include a reading of 100%.
